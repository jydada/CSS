...
def main():
    ...
    #4.5.4 load dataset
    train_dataloader = DataLoader(HaodaDataset(train_data_list),batch_size=config.batch_size,shuffle=True,collate_fn=collate_fn,pin_memory=True)
    val_dataloader = DataLoader(HaodaDataset(val_data_list,train=False),batch_size=config.batch_size,shuffle=True,collate_fn=collate_fn,pin_memory=False)
    ...
    #test(test_dataloader,model,fold)
        if (epoch + 1) % 10 == 0:
            filepath = config.weights + config.model_name + os.sep + str(epoch + 1) + os.sep
            filename = config.weights + config.model_name + os.sep + str(epoch + 1) + os.sep + "_checkpoint.pth.tar"
            if not os.path.exists(filepath):
                os.mkdir(filepath)
            state = {"epoch": epoch + 1, "model_name": config.model_name, "state_dict": model.state_dict(),
                     "best_precision1": best_precision1,
                     "optimizer": optimizer.state_dict(),
                     "fold": fold,
                     "valid_loss": valid_loss,
