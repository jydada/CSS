...
args = {
    'root'      : '',
    'data_path' : 'dataset/DRIVE3/',
    'epochs'    : 100,
    'lr'        : 0.0001,
    'snapshot'  : 10,
    'test_step' : 1,
    'ckpt_path' : 'checkpoint/20/',
    'batch_size': 5,
}
...
def train():
    ...
    for epoch in range(args['epochs']):
        ...
        if (epoch + 1) % args['test_step'] == 0:
            test_acc1, test_sen1 , test_iou1, test_acc2, test_sen2 , test_iou2,test_acc3, test_sen3, test_iou3,test_acc4, test_sen4 , test_iou4  = model_eval(net)
            print("Average acc1:{0:.4f}, average sen1:{1:.4f}, average iou1:{2:.4f}".format(test_acc1, test_sen1, test_iou1))
            print("Average acc2:{0:.4f}, average sen2:{1:.4f}, average iou2:{2:.4f}".format(test_acc2, test_sen2, test_iou2))
            print("Average acc3:{0:.4f}, average sen3:{1:.4f}, average iou3:{2:.4f}".format(test_acc3, test_sen3, test_iou3))
            print("Average acc4:{0:.4f}, average sen4:{1:.4f}, average iou4:{2:.4f}".format(test_acc4, test_sen4, test_iou4))
...
def model_eval(net):
    ...
    for idx, batch in enumerate(batchs_data):
        ...
        pred_val = net(image)
        acc1, sen1, iou1, acc2, sen2, iou2, acc3, sen3, iou3, acc4, sen4, iou4 = metrics(pred_val, label, pred_val.shape[0])
        ...
        return np.mean(Acc1), np.mean(Sen1), np.mean(IoU1),np.mean(Acc2), np.mean(Sen2), np.mean(IoU2),np.mean(Acc3), np.mean(Sen3), np.mean(IoU3),np.mean(Acc4), np.mean(Sen4), np.mean(IoU4)
