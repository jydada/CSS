...
class Attention2_block(nn.Module):
    """
    Attention Block
    """

    def __init__(self, F_g, F_l, F_int):
        super(Attention2_block, self).__init__()
        ...
        self.ci = nn.Sequential(
            nn.Conv2d(F_int*F_int, F_g, kernel_size=1, stride=1, padding=0, bias=True),
            nn.BatchNorm2d(F_g),
            nn.Sigmoid()
        )
    ...
    def forward(self, g, x):
        g1 = self.W_g(g)
        x1 = self.W_x(x)
        weights = self.relu(g1 + x1)

        psi = self.psi(weights)

        B, C, H, W = weights.size()
        proj_query = x.view(B, C, -1)
        proj_key = x.view(B, C, -1).permute(0, 2, 1)
        affinity = torch.matmul(proj_query, proj_key)
        affinity = affinity.view(B, -1, 1, 1)
        ci = self.ci(affinity)
...
class AttU_Net2(nn.Module):
    ...
    def __init__(self, img_ch=3, output_ch=1):
        ...
        self.Up5 = up_conv(filters[4], filters[3])
        self.Att5 = Attention2_block(F_g=filters[3], F_l=filters[3], F_int=filters[2])
        self.Up_conv5 = conv_block(filters[4], filters[3])

        self.Up4 = up_conv(filters[3], filters[2])
        self.Att4 = Attention2_block(F_g=filters[2], F_l=filters[2], F_int=filters[1])
        self.Up_conv4 = conv_block(filters[3], filters[2])

        self.Up3 = up_conv(filters[2], filters[1])
        self.Att3 = Attention2_block(F_g=filters[1], F_l=filters[1], F_int=filters[0])
        self.Up_conv3 = conv_block(filters[2], filters[1])

        self.Up2 = up_conv(filters[1], filters[0])
        self.Att2 = Attention2_block(F_g=filters[0], F_l=filters[0], F_int=32)
        self.Up_conv2 = conv_block(filters[1], filters[0])
...
class R2AttU_Net2(nn.Module):
    ...
    """
    def __init__(self, in_ch=3, out_ch=1, t=2):
        super(R2AttU_Net2, self).__init__()
...
class RAttU_Net(nn.Module):
    """
    Residual Block with attention Unet
    Implementation : https://github.com/LeeJunHyun/Image_Segmentation
    """
    def __init__(self, in_ch=3, out_ch=1):
        super(RAttU_Net, self).__init__()
        ...
        self.RCNN1 = res_block(in_ch, filters[0])
        self.RCNN2 = res_block(filters[0], filters[1])
        self.RCNN3 = res_block(filters[1], filters[2])
        self.RCNN4 = res_block(filters[2], filters[3])
        self.RCNN5 = res_block(filters[3], filters[4])

        self.Up5 = up_conv(filters[4], filters[3])
        self.Att5 = Attention_block(F_g=filters[3], F_l=filters[3], F_int=filters[2])
        self.Up_RCNN5 = res_block(filters[4], filters[3])

        self.Up4 = up_conv(filters[3], filters[2])
        self.Att4 = Attention_block(F_g=filters[2], F_l=filters[2], F_int=filters[1])
        self.Up_RCNN4 = res_block(filters[3], filters[2])

        self.Up3 = up_conv(filters[2], filters[1])
        self.Att3 = Attention_block(F_g=filters[1], F_l=filters[1], F_int=filters[0])
        self.Up_RCNN3 = res_block(filters[2], filters[1])

        self.Up2 = up_conv(filters[1], filters[0])
        self.Att2 = Attention_block(F_g=filters[0], F_l=filters[0], F_int=32)
        self.Up_RCNN2 = res_block(filters[1], filters[0])
