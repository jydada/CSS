...
#from utils.model_eval import evalen

DATABASE = './需统计/'
#
args = {
    'root'     : './dataset/' + DATABASE,
    'test_path': './dataset/' + DATABASE + 'val/',
    'pred_path': 'assets/' + 'baseline/100/',
    'img_size' : 512
}
...
def load_drive():
    ...
    for file in glob.glob(os.path.join(args['test_path'], 'images', '*.tif')):
        ...
        label_name = os.path.join(args['test_path'], 'images', basename)
        test_images.append(image_name)
        test_labels.append(label_name)
    return test_images, test_labels
...
def load_net():
    net = torch.load('./checkpoint-MoNuSeg/100/CS_Net_DRIVE_100.pkl')
    return net
...
def save_prediction(pred, filename=''):
    ...
    mask = (pred.data.cpu().numpy()).astype(np.uint8)
    #mask = np.transpose(np.squeeze(mask, axis=0), [1, 2, 0])
    mask = np.squeeze(mask, axis=0)
    mask = Image.fromarray(mask)
    mask.save(save_path + filename + '.png')
...
def predict():
    ...
    with torch.no_grad():
        ...
        for i in range(len(images)):
            ...
            save_prediction(output, filename=index)
